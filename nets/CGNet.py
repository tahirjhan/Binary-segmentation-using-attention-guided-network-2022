...
class ConvBlock(nn.Sequential):
    def __init__(self, in_channels: int, out_channels: int, kernel_size, stride: int = 1, padding: int = 0,
                 dilation: int = 1, groups=1, bias: bool = False, bn: bool = True, act: nn.Module = nn.PReLU()):
...
class CGBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, dilation: int, reduction: int = 16):
        ...
        # depthwise 3x3
        self.f_loc = nn.Conv2d(out_channels // 2, out_channels // 2, kernel_size=3, padding=1, groups=out_channels // 2,
                               bias=False)
        self.f_sur = nn.Conv2d(out_channels // 2, out_channels // 2, kernel_size=3, padding=dilation, dilation=dilation,
...
class CGBlock_Down(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, dilation: int, reduction: int = 16):
        super().__init__()
        # 1x1 conv
        self.downconv_3x3 = ConvBlock(in_channels, out_channels, 3, stride=2, padding=1, bias=False, bn=True,
                                      act=nn.PReLU())  # H->H/2, W->W/2
        # depthwise 3x3
        self.f_loc = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, groups=out_channels, bias=False)
        self.f_sur = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=dilation, dilation=dilation,
...
class CGNet(nn.Module):
    def __init__(self, in_channels: int, num_classes: int, M=3, N=21):
        ...
        self.stage_1 = nn.Sequential(
            ConvBlock(in_channels, 32, 3, stride=2, padding=1, bias=False, bn=True, act=nn.PReLU()),
    ...
    def forward(self, x):
        ...
        if self.last_activation is not None:
            output = self.last_activation(self.classifier(output3))
            output = F.interpolate(output, x.shape[-2:], mode='nearest')
        return output
